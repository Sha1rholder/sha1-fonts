...
| 文件 | 字重 | 字宽 | 命名实例 | 编码字符 |
| --- | --- | --- | --- | --- |
| `Sha1SansPatch-VariableFont_wdth,wght.ttf` | 100–900 | 62.5–100% | 36 | 19 |
| `Sha1SansPatch-Italic-VariableFont_wdth,wght.ttf` | 100–900 | 62.5–100% | 36 | 10 |
| `Sha1SansMonoPatch-VariableFont_wdth,wght.ttf` | 100–900 | 62.5–100% | 36 | 21 |
| `Sha1SerifPatch-VariableFont_wdth,wght.ttf` | 200–900 | 62.5–100% | 32 | 14 |
| `Sha1SerifPatch-Italic-VariableFont_wdth,wght.ttf` | 200–900 | 62.5–100% | 32 | 5 |
...
| Mono的`1` | 直接删除Noto Sans Mono数字1独立的底横轮廓，主体的形状、位置和步进保持不变 |
...
| 三个家族的`\|` | 以原中心为基准垂直延长10%，保持竖干宽度和步进，并添加突出竖干的居中实心圆 |
...
当前`lib.OUTLINE_DESIGN_VERSION`为3，旧占位轮廓缓存会被拒绝。后续修改设计时应同步更新生成、独立验证与轮廓版本，并完整重建字体
...
- 各中间位置的双线性轮廓和步进插值，以及艺术字`O`的独立几何约束
- 5个样式各98个拉丁字符的字体回退及400/700字重表现；源设计允许下划线保持固定笔画
- HarfBuzz对每个字体的浅、常规、重位置整形，检查补丁字符无缺字
- 原始字体和许可证哈希保持不变
- 20项单元与回归测试，包括艺术字`O`的五个样式、拒绝旧圆点占位和普通圆环、错误的Mono步进与直立Italic，以及旧缓存、点零来源、Mono数字1和破折号回归
